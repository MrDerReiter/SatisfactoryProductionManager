...
namespace SatisfactoryProductionManager.Model
{
    public class FileSaveLoadManager : IFactorySaveLoadManager
    {
        ...
        private static void RebuildProductionBlock(ProductionBlock block, List<ProductionUnit> controlUnitList)
        {
            ...
            {
                for (int j = 0; j < controlUnitList.Count; j++)
                {
                    if (block.Inputs[i] == controlUnitList[j].ProductionRequest)
                    {
                        block.AddProductionUnit(block.Inputs[i], controlUnitList[j].Recipe);
                        controlUnitList.RemoveAt(j);
                        i = -1;
                        break;
                    }
                }
